...
private final Boolean diffSpeeds = true;    // enable different speeds of movement  
...
/** 
  * Colour options
  **/ 
private final int minRGB = 200;  // min RGB value
private final int maxRGB = 250;  // max RGB value

private final int mindRGB = -5; // min change in RGB values
private final int maxdRGB = 5;  // max change in RGB values

private final int bg = 0;      // background colour (greyscale)


/**
  * Shape options 
  **/ 
private final int w = 100;  // width 
private final int h = 100;  // height
private final int fr = 10;  // framerate 

void setup(){
  size(300,300);     // must be same dimensions as w,h 
...
private final class point{ 
  ...
  public void oscillate(){
    ...
    if(this.moveRight){  
      ...
      else this.x +=  (int) (Math.pow(float(this.distx)-float(w/2),1/4)) * this.jump; 
    }  
    else{
      if(this.x == w/2) this.x -= 1; 
      else this.x -= (int) (Math.pow(float(this.distx)-float(w/2),1/4)) * this.jump;
   ...
   // vertical movement 
    if(this.moveUp){      
      ...
      else this.y +=  (int) (Math.pow(float(this.disty)-float(h/2),1/4)) * this.jump; 
    }  
    else{
      if(this.y == h/2) this.y -= 1; 
      else this.y -= (int) (Math.pow(float(this.disty)-float(h/2),1/4)) * this.jump;
